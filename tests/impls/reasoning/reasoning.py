...
@pytest.mark.parametrize(("mapname", "maxiters", "maxheap"), (
    ("complex_sleepless", 2000, 500),
    ("complex_nodebug", 2000, 500),
    # Interesting: looks like there's a time/space tradeoff in these cases:
    ("debug_complex", 5000, 3000),
...
@pytest.mark.parametrize(("mapname", "maxiters", "maxheap"), (
    # Interesting: looks like there's a time/space tradeoff in these cases:
    ("debug_complex", 10000, 5000),
    ("complex_nodebug_workhard", 10000, 5000),
))
def test_repeated_multigoal_foodrestdebug(mapname, maxiters, maxheap):
    start = State.fromdict({"HasCleanDishes": 1}, name="START")
